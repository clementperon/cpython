...
* `--disable-framework`
  ...
  of a `Python.framework`. This is also what you get by providing no framework
  argument at all. The configuration comes with significant restrictions; see
  [Building a static Python](#building-a-static-python) below.
...
`--disable-framework` selects a non-framework build, but it is optional:
omitting `--enable-framework` has the same effect. A non-framework build is
also static by default, as `--enable-shared` is off unless requested;
`--enable-shared` without a framework is rejected, since an iOS app can only
load a signed framework, never a bare dylib.
